...
# Sample Data
setwd("C:\\Users\\JHLee\\Desktop\\thesis")  # Varies from cmputer!!!
...
for (i in 1:simul){
  ...
  all_VAR <- matrix(0,10,10)
  all_VAR2 <- matrix(0,10,10)
  all_VAR3 <- matrix(0,10,10)
  all_VAR4 <- matrix(0,10,10)
  ...
  #     all_VAR[a,k] <- (all_variance[a,k])
  #   }
  # }
  # 
  # ak_max <- which(all_VAR == min(all_VAR), arr.ind = TRUE)    
  # 
  # a <- a_i[ak_max[1]]
  # k <- k_i[ak_max[2]]
  # 
  ...
  # 
   ...
   for (a in 1:10){
     for (k in 1:10){
       ...
       all_VAR_global[a,k] <- (all_variance_global[a,k])
     }
   }
  
   ak_max_global <- which(all_VAR_global == min(all_VAR_global), arr.ind = TRUE)
