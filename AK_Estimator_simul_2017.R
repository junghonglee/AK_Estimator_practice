...
# from 2000 Jan ~ 2015 Dec from ???계청 ????ف률 ?ـ???
setwd("C:\\Users\\JHL\\Desktop\\thesis")
unemployment <- read.csv("unemployment.csv", header=T)
population <- matrix(NA,15000,192)
popul <- NULL


#15000 ???의 population??? ?????? 그중 ??? 1???당 ?????? ???부 (1 or 0)
...
for (i in 1:simul){
  ...
  # Getting the beset value for a and k for minimum variance
  ...
  for (a in 1:10){
    for (k in 1:10){
      for (boo in 1:boot_simul) {
        ...
        ak_sim[t] <- 1/1500*(  (1-k_i[k]+a_i[a])*sum(data[new_input,t]) + (1-k_i[k]-(100/1400)*a_i[a])*sum(data[old_input,t])  )  + k_i[k]*(mean(data[sample_no[,t-1],t-1]) + delta[t])
      ...
      #all_mse[a,k] <- ((unemployment[[t]]/100) - boo_sum3)^2)/length(boo_sum3)
    }
  }
  ...
  ############################################
  # Testing for Simulation AK estimator 3 set#
  ############################################
  
  #fix value of a and k 
  a <- a_i[ak_max[1]]
  k <- k_i[ak_max[2]]
  delta <- 0
  ...
  ak_sim[t] <- 1/1500*(  (1-k+a)*sum(data[new_input,t]) + (1-k-(100/1400)*a)*sum(data[old_input,t])  )  + k*(mean(data[sample_no[,t-1],t-1]) + delta[t])
